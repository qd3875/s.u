(function(){
    ...
    window.smartInitHook=function(config){
        ...
        if(!webos){
            config.plugins.epub.push({js: rootPath +"/common/sdk/plugins.sdk.js"});
        }else{
            window.webos = parent.webos;
        }
        config.plugins.epub.push({js:rootPath+ "/apps/epub/js/index-init.js"});
        if(window.initVersion){
            utils.syncLoadData(rootPath+"/apps/epub/index.json?_="+Date.now(),function (text){

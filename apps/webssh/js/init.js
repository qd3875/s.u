(function(){
    ...
    window.smartInitHook=function(config){
        ...
        if(!webos){
            config.plugins.webssh.push({js: rootPath +"/common/sdk/plugins.sdk.js"});
        }else{
            window.webos = parent.webos;
        }
        config.plugins.webssh.push({js:rootPath+ "/apps/webssh/js/index-init.js"});
        if(window.initVersion){
            utils.syncLoadData(rootPath+"/apps/webssh/index.json?_="+Date.now(),function (text){

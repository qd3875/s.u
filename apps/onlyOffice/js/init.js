(function(){
    ...
    window.smartInitHook=function(config){
        ...
        var onlyHost = localStorage.getItem("onlyOfficeHost");
        config.plugins.office = [
            {js:onlyHost+"/web-apps/apps/api/documents/api.js"}
        ];
        var webos = null;
        try{
            webos = parent.webos;
        ...
        config.plugins.tabs.push({js:rootPath+ "/apps/onlyOffice/js/index-init.js"});
        config.plugins.office.push({js:rootPath+ "/apps/onlyOffice/index-init.js"});
        if(window.initVersion){
            utils.syncLoadData(rootPath+"/apps/onlyOffice/index.json?_="+Date.now(),function (text){

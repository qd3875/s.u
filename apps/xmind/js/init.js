(function(){
    ...
    window.smartInitHook=function(config){
        ...
        if(!webos){
            config.plugins.xmind.push({js: rootPath +"/common/sdk/plugins.sdk.js"});
        }else{
            window.webos = parent.webos;
        }
        config.plugins.xmind.push({js:rootPath+ "/apps/xmind/js/index-init.js"});
        if(window.initVersion){
            utils.syncLoadData(rootPath+"/apps/xmind/index.json?_="+Date.now(),function (text){

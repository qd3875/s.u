(function(){
    ...
    window.smartInitHook=function(config){
        ...
        config.plugins.birdpaper = [
            {css:rootPath+ "/apps/birdpaper/css/style.css"}
        ];
        var webos = null;
        try{
            webos = parent.webos;
        ...
        if(!webos){
            config.plugins.birdpaper.push({js: rootPath +"/common/sdk/plugins.sdk.js"});
        }else{
            window.webos = parent.webos;
        }
        config.plugins.birdpaper.push({js:rootPath+ "/apps/birdpaper/js/index-init.js"});
        if(window.initVersion){
            utils.syncLoadData(rootPath+"/apps/birdpaper/index.json?_="+Date.now(),function (text){

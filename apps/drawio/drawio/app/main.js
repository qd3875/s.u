(function(){
	...
	var fileReload = function(){
		...
		fetch(FILE_INFO.fileUrl).then(function(res){
			return res.blob();
		}).then(function (res){
			fileSetContent(res);
		});
	}
	var fileSaveCreate = async function(content){
		var fileName = FILE_INFO.fileName;
		var ext = fileExt(fileName);
		if( ext != 'drawio') {
			fileName = fileName.substr(0, fileName.length - ext.length - 1) + '.drawio';
		}
		parent.webos.inface.selectFileExt("folder",function(sz){
			if(sz.length != 1){
				return false;
			}
			(async function(){
	...
	var fileChangeStatus = function(){
		...
		if( !$save.length ){
			...
			var htmlSave = "<div class='draw-file-save' title='保存(ctrl+s)' style='display:none;'><span>保存</span></div>";
	...
	var menuReload = function(){
		...
		var menuFileLoad = function(dom){
			...
			var $html = $(html).insertAfter(dom.find('[item="new"]'));
			$html.bind('mousedown',function(e){
				e.preventDefault && e.preventDefault();
				return false;
			}).bind('mouseup',function(e){
				appMain.hideCurrentMenu();
				parent.webos.inface.selectFileExt("drawio,vsd,vsdx,vdx,graphml,pos,dio",function (sz){
					if(sz.length != 1){
						parent.webos.message.error("请选择一个文件后重试");
						return false;
					}
					var file = sz[0];
					(async function (){
						var url = await parent.webos.fileSystem.zl(file.path,2);
						var path = parent.webos.util.getParentPath(file.path);
						var canWrite = !path.startsWith("{sio:")
						fileOpen(url,file.name,path,canWrite);
					})();
					return true;
				});
			});
	...
	var appMainBeforeLoad = function(){
		...
		App.prototype.alert = function(msg){
			parent.webos.message.success(msg);
		}
		App.prototype.saveFile = function(){
			...
			fileThumbAppend(drawFile.getData(),async function(content){
				if(!FILE_INFO.savePath) return fileSaveCreate(content);
				var param = {
					parentPath:FILE_INFO.savePath,
		...
		var urlRepair = function(str){
			...
			str = str.replace("https://viewer.diagrams.net/?client=1&page=0&edit=_blank",WEB_HOST);
			str = str.replace("https://viewer.diagrams.net/?client=1&amp;page=0&amp;edit=_blank",WEB_HOST);
			str = str.replace("https://viewer.diagrams.net/?",BASE_URL_CDN
				+'index.html?lightbox=1&');
			str = str.replace("https://app.diagrams.net/?",BASE_URL_CDN
				+'index.html?lightbox=1&');
			return str;
		}
		// url兼容处理; 嵌入--链接--url;

(function(){
    ...
    window.smartInitHook=function(config){
        ...
        config.plugins.pdfjs = [
            {js:rootPath+ "/apps/pdfjs/js/pdf.js"},
            {js:rootPath+ "/apps/pdfjs/viewer.js"}
        ];
        var webos = null;
        try{
            webos = parent.webos;
        ...
        if(!webos){
            config.plugins.pdfjs.push({js: rootPath +"/common/sdk/plugins.sdk.js"});
        }else{
            window.webos = parent.webos;
        }
        config.plugins.pdfjs.push({js:rootPath+ "/apps/pdfjs/js/index-init.js"});
        if(window.initVersion){
            utils.syncLoadData(rootPath+"/apps/pdfjs/index.json?_="+Date.now(),function (text){

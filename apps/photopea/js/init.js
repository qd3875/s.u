(function(){
    ...
    window.smartInitHook=function(config){
        ...
        if(!webos){
            config.plugins.photopea.push({js: rootPath +"/common/sdk/plugins.sdk.js"});
        }else{
            window.webos = parent.webos;
        }
        config.plugins.photopea.push({js:rootPath+ "/apps/photopea/js/index-init.js"});
        if(window.initVersion){
            utils.syncLoadData(rootPath+"/apps/photopea/index.json?_="+Date.now(),function (text){

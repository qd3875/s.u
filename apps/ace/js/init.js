(function(){
    ...
    window.smartInitHook=function(config){
        ...
        config.plugins.ace = [
            {js:rootPath+"/apps/ace/js/boot.js"}
        ];
        var webos = null;
        try{
            webos = parent.webos;
        ...
        if(!webos){
            config.plugins.ace.push({js: rootPath +"/common/sdk/plugins.sdk.js"});
        }else{
            window.webos = parent.webos;
        }
        config.plugins.ace.push({js:rootPath+ "/apps/ace/js/index-init.js"});
        if(window.initVersion){
            utils.syncLoadData(rootPath+"/apps/ace/index.json?_="+Date.now(),function (text){

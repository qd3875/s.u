(function(){
    ...
    window.smartInitHook=function(config){
        ...
        config.plugins.album = [
            {js:rootPath+"/apps/album/js/album.js",css:rootPath+"/apps/album/css/style.css"}
        ];
        var webos = null;
        try{
            webos = parent.webos;
        ...
        if(!webos){
            config.plugins.album.push({js: rootPath +"/common/sdk/sdk.js"});
        }else{
            window.webos = parent.webos;
        }
        config.plugins.album.push({js:rootPath+ "/apps/album/js/index-init.js"});
        if(window.initVersion){
            utils.syncLoadData(rootPath+"/apps/album/index.json?_="+Date.now(),function (text){
